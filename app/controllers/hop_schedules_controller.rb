class HopSchedulesController < ApplicationController
  ...
  # POST /recipes.json
  def create
    @hop_schedule = HopSchedule.new(hop_schedule_params)

    respond_to do |format|
      if @hop_schedule.save
        format.html { redirect_to @hop_schedule, notice: 'Hop Schedule was successfully created.' }
        format.json { render :show, status: :created, location: @hop_schedule }
      else
  ...
  private
    ...
    def hop_schedule_params
      params.require(:hop_schedule).permit(:hop_id, :recipe_id, :amount, :at)
    end
end
